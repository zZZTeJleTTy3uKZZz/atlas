...
- `Project`: `+ sync_policy` (FK `SyncPolicy.slug`, default из типа), `+ backend_id?`.
...
- **F3b — доменная модель** (Ask First, бэкап): миграции Alembic (Epic, ChecklistItem, TaskMember, `backend_id`-поля, SyncPolicy, Outbox, SyncCursor) + сиды политик + `ProjectType.default_sync_policy`. Тесты миграций.
