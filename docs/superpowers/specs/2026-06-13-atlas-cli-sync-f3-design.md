...
| 6 | Де-хардкод классов «клиент/мой/Cifro» + «чьё пространство» | **C — гибрид:** owner-контрагент (`Counterparty`, данные) задаёт принадлежность + пространство синка; инфра-раскладка (группы/git-namespace) → `clikit.AppConfig` (конфиг, не зашито в `.py`). Тип проекта ортогонален owner. |
...
- `ProjectType`: `+ default_sync_policy` (FK `SyncPolicy.slug`). **Стартовые дефолты — это данные, Дмитрий финализирует (см. §12.6):** dev-типы (`personal-utility`/`personal-project`/`shared-infrastructure`/`business-product` — всё это разработка) → `local` или `epics`; `client-project` → `full`. «Медийки» отдельным типом в текущих сидах нет.
...
**Де-хардкод классов «клиент / мой / Cifro» (решение C — две ортогональные оси):**
Сейчас принадлежность зашита косвенно и в коде: `TYPE_TO_GROUP` (`pm/paths.py:39`), `TOP_LEVEL_GROUP="cifropro1"` (`pm/git_paths.py:33`), `GROUP_FOLDER_NAMES`, `archived_group`-enum, owner-теги. Разводим на две оси:
- **Бизнес-ось (данные):** `Project.owner` (`Counterparty`) выражает принадлежность ЯВНО вместо косвенного `type`+owner-тег. От owner вытекает **пространство синка** — Atlas узнаёт, «в чьё пространство» слать (Б24 Цифро / Б24 Артёма / Notion …). `customer` — заказчик.
- **Инфра-ось (конфиг):** зашитые маппинги групп/namespace → `clikit.AppConfig` (`config.toml`, слои global/project/local). Группа и git-namespace **выводятся** из `(type, owner)` по конфигу. Дефолты сохраняют текущее поведение (`cifropro1`) — junction-физику НЕ ломаем; `owner.git_namespace` переопределяет.
- **Тип проекта** остаётся осью «характер работы» (client/product/utility), ортогональной owner и политике. Принадлежность ≠ тип: клиентский Артёма и клиентский Цифро — один `type`, разные owner и пространства.
...
6. **Маппинг тип→дефолт-политика + «медийка»** (за Дмитрием): (а) синкать ли *эпики* dev-проектов наружу — `local` (вообще ничего, команда не видит даже вех) или `epics` (вехи видны, задачи нет); (б) нужен ли отдельный `ProjectType` `media` (политика `media` = эпик+задача, без чеклистов) или медийные проекты идут как `client-project`/`full`. Это данные-сиды, не код — меняются без миграции структуры.
