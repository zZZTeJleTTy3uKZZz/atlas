...
- Интейк идей: backlog — единый пул (convert → task | project со scaffolding); idea/inbox убраны как top-level (#867).
- Git/layout/backup портфеля; журнал/аудит (`log list` обогащённо · `log raw` сырой).
...
триажа) · `stats` (аналитика) · `config` (`show`/`get`/`set`/`setup`) ·
`update` (self-update с PyPI; `--from-git` — legacy git-reinstall) · `backend`
(`connect`/`disconnect`/`status` — внешний синк).
...
Статус задачи (`todo → in_progress → review → done`, плюс `blocked`/`cancelled`; сырые идеи ДО
задачи — в отдельном пуле `atlas backlog`) меняется намеренными ГЛАГОЛАМИ — они уважают lease (мультиагентную блокировку), а «голый»
...
- **`task add` / `update --status` принимают ТОЛЬКО `todo`** (планирование). Lifecycle-статусы
...
- **`atlas setup`** — turnkey-онбординг в агента ОДНОЙ командой: правила (как `init`) **+** SessionStart-хук
  для Claude Code (пишет `~/.claude/hooks/session_atlas.py` и идемпотентно мержит `~/.claude/settings.json`,
  НЕ трогая чужие хуки; хук гоняет `atlas task triage` и впрыскивает сводку портфеля в начало сессии).
  Флаги: `--no-rules` / `--no-hooks` / `--uninstall` / `--dry-run` / `--scope` / `--agents`. Установщики
  (`install.ps1` / `install.sh`) зовут `atlas setup` после установки CLI. Проверка: `atlas setup --dry-run`.
- **`atlas config setup`** — интерактивный визард конфига (owner, timezone, дефолты задач). Дефолты
...
atlas task checklist add <ref> --text "Прототип в Figma" --due 2026-06-25
...
1. **Источник правды — живой CLI.** Перед сложной командой сверься с `atlas <group> --help`. Команды в
   ...
   Интейк идей веди через backlog (единый пул); idea/inbox как top-level команды убраны (#867).
...
8. **audit read-only.** Только `atlas log raw` (обогащённо — `atlas log list`); таблица append-only.
...
11. **Статус — ГЛАГОЛАМИ, не `update --status`.** Взять в работу — `task start` (не `update --status
    ...
    `update --status` принимает лишь `todo`. Перед работой над задачей бери её через `start`
    (lease защищает от двойного захвата в мультиагентности).
12. **У каждого проекта — удалённый репо (обязателен).** `project add` по умолчанию `--no-init-git`
    (remote не создаётся) — доводи создание/онбординг до remote (`--init-git` / `project git init`),
    сверяй пропуски `project git status-all` (строки `URL —`). Существующий репо с историей → сохрани
    её (ручной `push` + `project git link`), НЕ `git init` (он схлопывает историю в один baseline).
    Открытие private→public — курируемая CI-джоба GitLab→GitHub, не зеркало. Детали —
    [references/projects-and-layout.md](references/projects-and-layout.md).
...
(`backup schedule install`) ставит пользователь в своей пользовательской сессии.
...
принимает лишь `todo`. Это защищает lease от обхода.
