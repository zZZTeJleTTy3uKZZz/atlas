...
глаголами** + lease, `--cpp` обязателен) · `epic` · `checklist` (пункты задачи) · `member` (роли
на задаче) · `hypothesis` (ledger) · `participant` (люди портфеля) · `type` / `status` / `tag`
(справочники) · `idea` / `inbox` (инкубатор/свалка) · `action-log` (аудит) · `backup`.
...
У задачи есть **reviewer** (по умолч. создатель = `ATLAS_ACTOR`/owner; задать `--reviewer <slug>`, снять
`--no-review`). **Закрыть в `done` может ТОЛЬКО reviewer** — исполнитель сдаёт на проверку:
