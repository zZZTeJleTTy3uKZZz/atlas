...
description: |
  ...
  <example>
  Context: проект `my-api-service-test` перешёл из status=experiment в
  кандидата на business-product, метаданные устарели, теги неполные.
  user: "Переинициализируй my-api-service-test как product"
...
- **Atlas** — CLI для управления локальным портфелем проектов.
...
- `owner:personal` vs `owner:organization` — определяется тем чьи это проекты
  (личные проекты пользователя → personal; общие/командные продукты → organization).
...
Этот проект зарегистрирован в локальной PM-системе Atlas:
...
2. **slug в Atlas менять нельзя** — это часть task IDs. Если slug
   фундаментально неправильный — фиксируй как открытый вопрос для пользователя,
...
4. **Не делай git push / git commit** в проект. Только `git status` для
   проверки состояния. Реальный push делает либо пользователь вручную, либо
