...
3. **Двигай статус ГЛАГОЛАМИ.** Взять задачу — `task start`, завершить — `task done`, прочее —
   `task review/block/unblock/cancel` (НЕ `update --status` — он лишь backlog\|todo). Перед `project archive`
   покажи состояние и подтверди. Перенос даты по явной просьбе — без подтверждения.
