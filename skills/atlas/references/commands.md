...
- `atlas sync allowlist set --project <slug> [--project <slug> …]` — разрешить исходящий синк только выбранным проектам. `atlas sync allowlist add <slug>` — позднее подключить ещё один; `list` — показать выбор. Пустой список означает прежний режим без отбора.
- `atlas sync seed --project <slug> [--project <slug> …] [--dry-run] [--include-closed]` — поставить открытые задачи указанных проектов в outbox; `--include-closed` догружает завершённую историю. Затем повторять `atlas sync push`, пока `sent` не станет 0. Команда не повторяет старую failed-очередь и не переносит проекты вне списка.
- `atlas sync bootstrap --dry-run|--yes` — для новой машины перейти к концу старой адресной ленты без применения истории; только после резервной копии БД и перед `seed` выбранных проектов. Чужие исторические события этим явно пропускаются.
- `atlas project update <slug> --sync-policy local|epics|media|full` — изменить уровень будущих исходящих событий без изменения владельца или видимости проекта.
...
## `atlas comm` — точки коммуникации

Встречи, созвоны, переписка, заметки — и то, что из них вынули. Отвечает на
вопрос, которого база раньше не понимала: **что вообще было по этому клиенту**.

Три уровня, потому что одна встреча кормит несколько проектов, а разговор о
клиенте идёт не подряд: коммуникация → её куски по проектам → факты из куска.

```sh
atlas comm add --title "Синхрон с Артёмом" --kind meeting --side internal     --date 2026-09-05 --source "docs/встречи/2026-09-05.md"
atlas comm part <id> --project hpl-piter --subject "Стас" --tiles 120-194
atlas comm fact <part-id> --kind decision --text "начинать с распознавания"     --quote "начинаем с распознавания, эдо потом"
atlas comm link <fact-id> --task 2644     # только когда задачу ДЕЙСТВИТЕЛЬНО завели
atlas comm list --project hpl-piter       # что было по клиенту
atlas comm list --counterparty slava --side client
atlas comm show <id|часть заголовка>      # куски по проектам и факты каждого
atlas comm search "каскадные рассылки"    # по тексту куска, заголовку, выжимке
atlas comm import digests.json --title "Встреча 05.09" --date 2026-09-05     --map карта.json                      # готовый разбор: куски и факты разом
```

Виды: `--kind meeting|call|chat|voice_note|presentation`, `--side internal|client`,
у факта `--kind status|decision|checkpoint|risk|task|open`.

Текст расшифровки в базу не переносится: он живёт файлом рядом с проектом
(`--source`), база держит структуру и путь. У факта дословная `--quote` — по ней
разбор проверяется поиском по исходнику, без участия человека; она же выдаёт
криво проведённую границу, если нашлась в соседнем куске.

Карта для `import` — это «как назвали вслух → слаг проекта»: вывести, что
«Медцентр» — это `med-persona`, из текста нельзя, знание человека.
