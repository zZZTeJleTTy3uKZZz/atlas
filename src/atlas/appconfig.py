...
class AtlasConfig(AppConfig):
    ...
    scope: str = "all"
    # Пусто = прежний режим без отбора; непустой список ограничивает исходящие
    # события проектами, которые владелец явно включил в синхронизацию.
    sync_projects: list[str] = []
    ...
    personal_owner: str = ""
    # GitHub owner (user/org) по умолчанию для `git init --provider github`.
    # Пусто → резолвится через `gh api user` (текущий аутентифицированный аккаунт).
    github_owner: str = ""
    ...
    # Корневая папка портфеля проектов (где лежат репозитории + junction-layout).
    # Пусто → env ATLAS_PROJECTS_ROOT › дефолт ~/Documents/PROJECT (см. paths.get_projects_root).
    projects_root: str = ""
...
def resolve_api_key(cfg: AtlasConfig) -> str:
    ...
    if legacy:
        ...
        # ВАЖНО: сохраняем ПОЛНЫЙ дамп (owner/namespaces/дефолты задач и пр.) —
        # иначе 5-польный конструктор затирал все прочие поля global config.toml.
