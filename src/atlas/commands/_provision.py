...
def resolve_project_mode(
    *,
    ...
    Владелец (counterparty): ``--owner`` > (``--team`` → ``company_owner`` из
    конфига) > ``default_owner``. lead (member-человек, видит все задачи): явный
    ``--owner`` (он же ведёт), иначе ``default_owner`` — даже у командного проекта
    (--team) lead остаётся человеком, компания lead'ом быть не может. Личный режим
    = НЕ team И (owner не задан ИЛИ owner == default_owner) → full + personal;
    иначе team + media. ``company_owner`` — config-driven (раньше хардкод орг-владельца).
    """
    explicit_owner = owner or (company_owner if (team and company_owner) else None)
