...
from atlas.paths import IDEAS_FOLDER_NAME, get_projects_root
...
ideas_app = typer.Typer(
    ...
    help="Идеи ПРОЕКТНОГО уровня (entity_kind=idea, → проект). Лёгкие идеи-в-задачи — `atlas backlog`.",
)
console = Console()
...
def add_cmd(
    ...
    with make_session(engine) as session:
        ...
        if tags:
            ...
            # owner-теги — по КАТЕГОРИИ (не по хардкод-набору значений): generic.
            owner_tags = [t.slug for t in resolved_tags if t.category == "owner"]
...
def demote_cmd(
    ...
    with make_session(engine) as session:
        ...
        from atlas.commands.projects import _hard_delete_physical
        from atlas.layout import get_logical_path, get_storage_path
