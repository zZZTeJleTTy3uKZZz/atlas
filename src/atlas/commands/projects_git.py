...
from atlas.git_backend import LocalGitOps, get_backend
from atlas.git_paths import derive_group_path, resolve_github_owner
...
def _resolve_group_for_provider(
    session: Session,
    project: Project,
    *,
    provider: str,
    ...
    """Определить namespace (group/owner) для создания remote под провайдер.

    - ``--group`` задан явно → используется как есть (любой провайдер).
    - ``gitlab`` → ``derive_group_path`` (вложенные подгруппы по type/status/tags).
    - ``github`` → owner из ``resolve_github_owner`` (config.github_owner / gh user).

    Raises ``RuntimeError`` если namespace определить не удалось.
    """
    ...
    if provider == "github":
        ...
        if not owner:
            raise RuntimeError(
                "GitHub: owner не определён. Укажи `--group <owner>`, задай "
                "`atlas config set github_owner <owner>` или выполни `gh auth login`."
            )
        return owner
    # gitlab (исторический дефолт): вложенная раскладка.
    ...
    return derive_group_path(
        pt.slug, ps.slug, project.archived_group, owner_tags=owner_slugs,
    )
...
def perform_git_init(
    ...
    *,
    ...
    group_path = _resolve_group_for_provider(
        session, project, provider=provider, group=group,
    )
...
def init_cmd(
    ...
    """Инициализировать git-репо проекта: local init + remote create + push.

    ``--provider gitlab`` (glab, вложенные группы) или ``github`` (gh, owner).
    """
    ...
    emit_data(
        ...
        text_renderer=lambda d: (
            ...
            console.print(f"  Provider: {d['provider']}"),
...
def _repo_full_path_from_url(url: str) -> str:
    ...
    Поддерживает:
        https://gitlab.com/<org>/clients/<repo>      -> <org>/clients/<repo>
        https://gitlab.com/<org>/clients/<repo>.git  -> <org>/clients/<repo>
        git@gitlab.com:<org>/clients/<repo>.git      -> <org>/clients/<repo>
    """
    s = url.strip()
...
def move_cmd(
    ...
    to_group: str = typer.Option(
        ..., "--to-group",
        help="Новый namespace: GitLab group path или GitHub owner (по провайдеру проекта).",
    ),
) -> None:
    """Перенести репо в другую группу/owner + обновить локальный remote и БД."""
    engine = make_engine(_db_url())
    with make_session(engine) as session:
        project = _resolve_project_or_die(session, ref)
...
def sync_from_remote_cmd(
    ...
    with make_session(engine) as session:
        ...
        for proj in projects:
            ...
            try:
                backend = get_backend(proj.git_provider or "gitlab")
