"""CLI-команды `atlas task ...` — CRUD по задачам портфеля (Atlas).
...
def _validate_planning_status(status: str) -> None:
    """add / update --status принимают ЛИШЬ planning-статус `todo`.

    Lifecycle-статусы (in_progress/review/done/blocked/cancelled) меняются только
    намеренными глаголами (task start/review/block/unblock/done/cancel) — чтобы
    «голый» статус не обходил lease (мультиагентная координация). Идеи-уровень
    (бывший backlog) переехал в отдельный пул `atlas backlog`."""
    from atlas.task_status import (
        LIFECYCLE_STATUSES,
        PLANNING_STATUSES,
        VERB_FOR_STATUS,
    )
...
def add_cmd(
    ...
    priority: Optional[str] = typer.Option(
        None, "--priority", help="P0 | P1 | P2 | P3 (по умолч. — config.default_priority)",
    ),
    status: str = typer.Option(
        "todo", "--status",
        help="Начальный статус: todo (деф., «запланировано»). in_progress/done и пр. — "
             "глаголами (task start/done/...). Идеи-уровень — пул `atlas backlog`.",
    ),
    ...
    reviewer: Optional[str] = typer.Option(
        None, "--reviewer",
        help="Кто проверяет/закрывает (slug). По умолч. — создатель (env ATLAS_ACTOR / "
             "owner). Только reviewer переводит в done; исполнитель — 'task submit'.",
    ),
    no_review: bool = typer.Option(
        False, "--no-review", help="Без reviewer-гейта (задача без приёмки).",
    ),
    ...
    _cfg = _load_cfg()
    priority = priority or _cfg.default_priority or "P2"  # дефолт из конфига
    _validate_priority(priority)
    _validate_planning_status(status)
    ...
    with make_session(engine) as session:
        ...
        # ----- reviewer: --no-review снимает; явный --reviewer; иначе — конфиг -----
        # (default_review + default_reviewer; default_reviewer пуст → создатель/actor).
        reviewer_id: Optional[str] = None
        _want_reviewer = (not no_review) and (reviewer is not None or _cfg.default_review)
        if _want_reviewer:
        ...
        data = {
            "id": task.id, "number": number, "slug": final_slug,
            "slug_auto": slug_auto, "title": title, "status": status,
            "priority": priority, "assignee": assignee, "project": proj.slug,
            "story_points": story_points, "epic": epic,
            "reviewer": _slug_for_assignee(session, reviewer_id),
        }

        def _render(d: dict[str, Any]) -> None:
            if d["slug_auto"]:
                console.print(f"[dim]slug auto-generated: {d['slug']}[/dim]")
            ass = f" · assignee: {d['assignee']}" if d["assignee"] else ""
            console.print(
                f"[green]✓ Task #{d['number']} '{d['slug']}' created[/green] · "
                f"{d['title']} · {d['status']} · {d['priority']}{ass}"
            )
...
# --------------------------------------------------------------------------- #
# batch — массовое создание задач (defaults на батч + override на задачу)      #
# --------------------------------------------------------------------------- #


def _create_one_task(session: Session, cfg, spec: dict, *, idx: int) -> dict[str, Any]:
    """Создать одну задачу из merged-spec (поле задачи › defaults › config › система).
    ...
    if proj.prefix is None:
        raise CliError("no_prefix", f"#{idx}: у проекта '{proj.slug}' нет prefix.")

    priority = str(spec.get("priority") or cfg.default_priority or "P2")
    _validate_priority(priority)
    status = str(spec.get("status") or "todo")
    _validate_planning_status(status)

    # slug: явный → validate+build; иначе авто из title
    explicit = spec.get("slug")
    if explicit:
        _validate_slug_part(str(explicit))
        final_slug = build_task_slug(proj.prefix, str(explicit))
        if _slug_exists_fn(session)(final_slug):
            raise CliError("slug_taken", f"#{idx}: slug '{final_slug}' занят.")
        slug_auto = False
    else:
        base = slugify_text(str(spec["title"]))
        if not base:
            raise CliError("slug_gen", f"#{idx}: не сгенерить slug из title — задайте slug.")
        final_slug = generate_unique_slug(build_task_slug(proj.prefix, base), _slug_exists_fn(session))
        slug_auto = True

    # reviewer: no_review снимает; reviewer явный; иначе config
    reviewer_id: Optional[str] = None
    rev = spec.get("reviewer")
    if not spec.get("no_review", False) and (rev or cfg.default_review):
        from atlas import lease as L
        try:
            reviewer_id = L.resolve_actor(session, rev or (cfg.default_reviewer or None)).id
        except L.LeaseError as exc:
    ...
    task = Task(
        number=next_task_number(session), slug=final_slug, project_id=proj.id,
        epic_id=epic_id, assignee_id=assignee_id, reviewer_id=reviewer_id,
        title=str(spec["title"]), description=spec.get("description"),
        cpp_description=str(spec["cpp"]), status=status, priority=priority,
        story_points=spec.get("story_points"), due_date=due_dt,
        quality_tier=spec.get("quality_tier"), origin="native",
    )
    session.add(task)
    session.flush()
    _log_action(session, action="task_created", entity_id=task.id,
                details={"number": task.number, "slug": final_slug, "project": proj.slug,
                         "title": task.title, "priority": priority, "status": status, "batch": True})
    try:
        _outbox.enqueue(session, "create", "task", task, project=proj, portal_id=_sync_portal_id())
    except Exception:
        pass
    return {"number": task.number, "slug": final_slug, "title": task.title,
            "project": proj.slug, "priority": priority, "status": status,
            "reviewer": _slug_for_assignee(session, reviewer_id), "slug_auto": slug_auto}


@task_app.command("batch")
@command
def batch_cmd(
    file: str = typer.Argument(..., help="TOML: [defaults] (на батч) + [[task]] (с override)."),
    dry_run: bool = typer.Option(False, "--dry-run", help="Показать, что создастся, без записи."),
) -> None:
    """Массовое создание задач из TOML. [defaults] — общие настройки батча, [[task]] —
    задачи (любое поле override'ит default). Разрешение: задача › defaults › config › система.
    ...
    p = Path(file)
    if not p.exists():
        raise CliError("not_found", f"Файл не найден: {file}")
    try:
        doc = tomllib.loads(p.read_text(encoding="utf-8"))
    except tomllib.TOMLDecodeError as exc:
        raise CliError("invalid_toml", f"Ошибка TOML: {exc}")
    defaults = doc.get("defaults") or {}
    items = doc.get("task") or []
    if not items:
        raise CliError("empty", "Нет ни одной [[task]] в файле.")
    cfg = _load_cfg()
    ...
    with make_session(engine) as session:
        for i, item in enumerate(items, 1):
            spec = {**defaults, **(item or {})}
            if not spec.get("title") or not spec.get("cpp"):
                raise CliError("invalid_item", f"[[task]] #{i}: обязательны title и cpp.")
            if dry_run:
                rows.append({"idx": i, "title": spec["title"], "project": spec.get("project"),
                             "priority": spec.get("priority") or cfg.default_priority,
                             "would_create": True})
            else:
                rows.append(_create_one_task(session, cfg, spec, idx=i))
        if not dry_run:
            session.commit()

    def _render(d: dict[str, Any]) -> None:
        head = "[yellow]dry-run[/yellow] " if d["dry_run"] else ""
        console.print(f"{head}[bold]Батч: {d['count']} задач(и)[/bold]")
        for r in d["tasks"]:
            mark = "·" if d["dry_run"] else "✓"
            console.print(f"  {mark} #{r.get('number', '?')} {r.get('slug', r['title'])} "
                          f"[grey50]{r.get('priority')}[/grey50] — {r['title']}")

    emit_data({"count": len(rows), "dry_run": dry_run, "tasks": rows}, text_renderer=_render)
...
def list_cmd(
    ...
    sprint: Optional[str] = typer.Option(
        None, "--sprint", help="Sprint ref (slug | UUID) — фильтр по sprint_id.",
    ),
    source_project: Optional[str] = typer.Option(
...
def _render_task_list(data: list[dict[str, Any]]) -> None:
    ...
    table = Table(title=f"Задачи ({len(data)})")
...
def get_cmd(
    ...
    from atlas.task_review import list_comments as _list_comments
    ...
    data = {
        ...
        "reviewer": _slug_for_assignee(session, task.reviewer_id),
        "epic": epic_slug,
        ...
        # комментарии (передача контекста между агентами + история review)
        "comments": [
            {
                "kind": c.kind,
                "author": _slug_for_assignee(session, c.author_id),
                "body": c.body,
                "created_at": c.created_at.isoformat() if c.created_at else None,
            }
            for c in _list_comments(session, task)
        ],
...
def update_cmd(
    ...
    with make_session(engine) as session:
        ...
        # status — ТОЛЬКО planning-переход (todo); валидация выше уже
        # отсекла lifecycle-статусы (они идут через глаголы task start/done/...).
        if status is not None and task.status != status:
            old_status = task.status
            diffs["status"] = {"old": old_status, "new": status}
            task.status = status
            # Реоткрытие из терминального (done/cancelled) в planning → снять
            # completed_at (задача снова в работе/плане).
            if old_status in ("done", "cancelled") and task.completed_at is not None:
