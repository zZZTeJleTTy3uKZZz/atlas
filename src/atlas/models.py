...
class Task(Base):
    ...
    # sprint_id — тайм-бокс Scrum (отдельно от epic_id = тематическая группировка).
    ...
    # reviewer_id — кто проверяет и закрывает задачу (по умолч. создатель). Гейт:
    # при заданном reviewer задачу в done переводит ТОЛЬКО он (исполнитель делает
    # submit→review). Review-workflow для мультиагентной приёмки.
    ...
    __table_args__ = (
        CheckConstraint(
            "status IN ('todo','in_progress','review','done','blocked','cancelled')",
...
class Sprint(Base):
    """Спринт — тайм-бокс (Scrum). В ОТЛИЧИЕ от эпика (тематическая группировка):
    спринт ограничен датами, в него набирают задачи из бэклога, по итогу считают
    velocity (сумма story_points завершённых задач). Локальный, lease не несёт."""
    ...
    planned_velocity: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    retro_notes: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=local_now, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(
        DateTime, default=local_now, onupdate=local_now, nullable=False
    )
...
class BacklogItem(Base):
    """Пул `backlog` — сырая идея/интейк ДО задачи (DB-first, лёгкая запись).

    Отличие от задачи: ЦКП НЕ обязателен, проект опционален (global-пул «между
    проектами»). Идея просматривается и **преобразуется** в задачу (`todo`) или
    зачаток проекта. Опционально материализуется в MD (`md_path`). Уровень,
    освободившийся после удаления статуса `backlog` у задач.
    """
    ...
    # null = global пул (между проектами); иначе привязка к проекту.
    project_id: Mapped[Optional[str]] = mapped_column(
        String(36), ForeignKey("projects.id"), nullable=True
    )
    priority: Mapped[Optional[str]] = mapped_column(String(3), nullable=True)
    md_path: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)
    # жизнь самой идеи: open → converted | archived.
    status: Mapped[str] = mapped_column(
    ...
    __table_args__ = (
        ...
        Index("idx_backlog_items_status", "status"),
    )


class Issue(Base):
    """Issue/Handoff — структурированная жалоба/передача (богатая обратная связь).

    Главный кейс — **передача задачи агент→агент** (kind='handoff'): тело по
    шаблону issuekit (что сделано / осталось / как проверить / ЦКП / контекст),
    валидируется БЛОКИРУЮЩЕ через ``issuekit.lint`` перед записью — неполную не
    создаём (симметрично обязательному ЦКП задачи). Также bug/feature как
    структурированные жалобы. Опционально привязан к задаче (``task_id``).
    """
    ...
    kind: Mapped[str] = mapped_column(String(20), nullable=False)  # bug|feature|handoff
    title: Mapped[str] = mapped_column(String(500), nullable=False)
    body: Mapped[str] = mapped_column(Text, nullable=False)  # markdown по шаблону issuekit
    status: Mapped[str] = mapped_column(
        String(20), default="open", server_default="open", nullable=False
    )
    # автор (кто сдал) и адресат (кому передаём, для handoff).
    ...
    __table_args__ = (
        CheckConstraint(
            "kind IN ('bug','feature','handoff')", name="ck_issues_kind",
        ),
        CheckConstraint(
            "status IN ('open','resolved','wontfix')", name="ck_issues_status",
        ),
...
class Comment(Base):
    """Комментарий к задаче (передача контекста между агентами + история review).

    В ОТЛИЧИЕ от action_log (авто-аудит CRUD): это намеренные заметки людей/агентов
    — что сделано, что зарелизено, что дальше; обоснование reject/approve/reopen.
    ``kind`` помечает связь с переходом (comment|submit|approve|reject|reopen)."""
    ...
    kind: Mapped[str] = mapped_column(
        String(20), default="comment", server_default="comment", nullable=False
    )
    created_at: Mapped[datetime] = mapped_column(DateTime, default=local_now, nullable=False)
