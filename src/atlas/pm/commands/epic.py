...
_DEFAULT_ACTOR_SLUG = "dmitry"


def _sync_portal_id() -> str:
    """Slug портала-стора этого Atlas для source_portal_id событий синка — из
    активного конфига (cfg.portal_id), а НЕ хардкод (как в pm_tasks/checklist).
...
def add_cmd(
    ...
    if origin is not None and origin not in VALID_ORIGINS:
        print(f"Невалидный origin '{origin}': допустимы {sorted(VALID_ORIGINS)}.")
        raise typer.Exit(1)
    ...
    with make_session(engine) as session:
        ...
        if proj is None:
            print(f"Project '{project}' не найден.")
            raise typer.Exit(1)
        ...
        if source_project is not None:
            src = resolve_project_ref(session, source_project)
            if src is None:
                print(f"Source project '{source_project}' не найден.")
                raise typer.Exit(1)
            if src.id == proj.id:
                # self-inject — это не инжект: warning, остаёмся native.
                print(
                    "Предупреждение: source-project совпадает с целевым — "
                    "это не инжект, origin остаётся 'native'."
                )
            else:
                source_id = src.id
                if origin is None:
