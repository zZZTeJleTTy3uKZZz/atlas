...
через pure-logic ``atlas.pm.stats`` (без нового стора).
...
from atlas.pm import stats as _stats
...
# --------------------------------------------------------------------------- #
# dashboard (#132) — объединённый обзор                                        #
# --------------------------------------------------------------------------- #


def _render_dashboard(d: dict[str, Any]) -> None:
    console.print("[bold magenta]═══ Atlas Dashboard ═══[/bold magenta]")
    console.print()
    _render_counts(d["counts"])
    console.print()
    _render_activity(d["activity"])
    console.print()
    _render_provenance(d["provenance"])
    if d.get("git") is not None:
        console.print()
        _render_git(d["git"])
...
def dashboard_cmd(
    period: str = typer.Option(
        "30d", "--period",
        help="Окно активности для секции activity (дефолт 30d).",
    ),
    project: Optional[str] = typer.Option(
        None, "--project",
        help="Добавить git-секцию по проекту (slug | UUID).",
    ),
    type_slug: Optional[str] = typer.Option(
        None, "--type", help="Фильтр activity по типу проекта.",
    ),
    counterparty: Optional[str] = typer.Option(
        None, "--counterparty", help="Фильтр activity по контрагенту-владельцу.",
    ),
    tag: Optional[str] = typer.Option(
        None, "--tag", help="Фильтр activity по тегу проекта (slug).",
    ),
) -> None:
    """Объединённый обзор портфеля: counts + activity + provenance (+ git)."""
    try:
        start, end = _stats.parse_period(period)
    except ValueError as exc:
        raise CliError("invalid_period", str(exc))
    ...
    engine = make_engine(_db_url())
    git_section: Optional[dict[str, Any]] = None
    pushed: Optional[str] = None
    with make_session(engine) as session:
        counts = _stats.project_counts(session)
        activity = _stats.activity_window(
            session, start=start, end=end,
            type_slug=type_slug, owner_slug=counterparty,
            tag_slug=tag,
        )
        provenance = _stats.provenance_stats(session)
        if project is not None:
            proj = _resolve_project_or_die(session, project)
            local_path = proj.local_path
            proj_slug = proj.slug
            pushed = proj.git_last_pushed_at.isoformat() if proj.git_last_pushed_at else None
    if project is not None:
        git_section = _stats.git_stats(local_path, last_pushed_at=pushed) or {
            "is_git": False, "path": None, "commits": 0, "last_pushed_at": pushed,
        }
        git_section = {"project": proj_slug, **git_section}
    ...
    emit_data(data, text_renderer=_render_dashboard)
