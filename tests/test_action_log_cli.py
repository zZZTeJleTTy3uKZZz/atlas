...
@pytest.fixture()
def app():
    from atlas.commands.action_log import app as action_log_app
...
class TestList:
    ...
    def test_list_filter_actor(self, runner, app, seeded_engine):
        ...
        with make_session(seeded_engine) as session:
            ...
            _add_log(session, actor_id=owner.id, entity_type="project",
                     entity_id="x", action="owner_action", details_json="{}")
            _add_log(session, actor_id=claude.id, entity_type="project",
                     entity_id="y", action="claude_action", details_json="{}")
            session.commit()

        result = runner.invoke(app, ["list", "--actor", "owner"])
        assert result.exit_code == 0
        assert "owner_action" in result.stdout
    ...
    def test_list_filter_project(
        self, runner, app, seeded_engine, projects_app, task_app,
    ):
