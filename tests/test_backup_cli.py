...
@pytest.fixture()
def app():
    from atlas.commands.backup import backup_app
    return backup_app
...
def make_project(seeded_engine, tmp_path):
    ...
    from atlas.db import make_session
    from atlas.models import Project, ProjectStatus, ProjectType
...
class TestBackupRun:
    ...
    def test_run_skips_projects_with_no_git_repo_url(
        ...
        with patch("atlas.commands.backup.backup_repo") as mock_backup:
            result = runner.invoke(app, ["run"])
            assert result.exit_code == 0
...
class TestBackupInstall:
    def test_install_invokes_powershell_register_task(self, runner, app, seeded_engine):
        ...
        with patch("atlas.commands.backup.subprocess.run") as mock_run:
