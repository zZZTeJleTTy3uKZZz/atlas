...
def task_ref(runner, app, projects_app, seeded_engine):
    ...
    # --no-review: lease-тесты не про приёмку; иначе reviewer=owner гейтит done
    # исполнителем claude-code (это покрыто отдельно в test_task_review).
    r = runner.invoke(
        app, ["add", "--project", "cifro", "--title", "T1", "--cpp", "ЦКП", "--no-review"]
    )
    assert r.exit_code == 0, r.stdout
    from atlas.db import make_session
...
def test_done_releases_lease(runner, app, task_ref):
    # status=done теперь через глагол `task done` (а не update --status),
    # который атомарно снимает lease.
    runner.invoke(app, ["claim", task_ref, "--actor", "claude-code"])
    r = runner.invoke(app, ["done", task_ref, "--actor", "claude-code"])
    assert r.exit_code == 0, r.stdout
