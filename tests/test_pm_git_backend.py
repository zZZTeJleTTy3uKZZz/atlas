"""Тесты для atlas.pm.git_backend.
...
class TestLocalGitOps:
    def test_init_runs_git_init(self, tmp_path, fake_runner):
        from atlas.pm.git_backend import LocalGitOps
    ...
    def test_add_remote_invokes_git_remote_add(self, tmp_path, fake_runner):
        ...
        ops = LocalGitOps()
        ops.add_remote(tmp_path, "origin", "git@gitlab.com:cifropro1/clients/cifro.git")

        cmds = [c["cmd"] for c in fake_runner["calls"]]
        assert any(
            c[:3] == ["git", "remote", "add"]
            and "origin" in c
            and "git@gitlab.com:cifropro1/clients/cifro.git" in c
            for c in cmds
        )
...
class TestGitLabBackend:
    ...
    def test_get_remote_status_returns_dict(self, fake_runner):
        ...
        # `glab repo view cifropro1/clients/cifro -F json` → JSON.
        fake_runner["set"](
            (
                0,
                '{"web_url":"https://gitlab.com/cifropro1/clients/cifro","default_branch":"main","visibility":"private"}',
                "",
            ),
        )
        backend = GitLabBackend()
        info = backend.get_remote_status("cifropro1/clients/cifro")
...
def test_gitlab_backend_satisfies_git_backend_protocol():
    ...
    from atlas.pm.git_backend import GitBackend, GitLabBackend
