...
def test_release_not_owner_exits_nonzero(runner, app, task_ref):
    runner.invoke(app, ["claim", task_ref, "--actor", "claude-code"])
    r = runner.invoke(app, ["release", task_ref, "--actor", "dmitry"])
    assert r.exit_code != 0
...
def test_take_requires_force(runner, app, task_ref):
    runner.invoke(app, ["claim", task_ref, "--actor", "claude-code"])
    # без --force — отказ
    assert runner.invoke(app, ["take", task_ref, "--actor", "dmitry"]).exit_code != 0
    # с --force — отбирает
    r = runner.invoke(app, ["take", task_ref, "--actor", "dmitry", "--force"])
    assert r.exit_code == 0, r.stdout
    d = json.loads(r.stdout)
    assert d["lease_owner"] == "dmitry"
