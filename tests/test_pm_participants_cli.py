...
def app():
    ...
    from atlas.pm.commands.participants import app as participants_app
...
class TestAdd:
    ...
    def test_add_slug_collision(self, runner, app, seeded_engine):
        """seed уже содержит 'dmitry' → попытка занятого slug → error."""
        result = _add(
            runner, app,
            "--name", "Other Dmitry",
            "--kind", "human",
        ...
        assert "dmitry" in combined.lower()
...
class TestList:
    def test_list_all_default(self, runner, app, seeded_engine):
        """Seed имеет 2 активных participants — dmitry и claude-code."""
        result = runner.invoke(app, ["list"])
        assert result.exit_code == 0, _combined(result)
        assert "dmitry" in result.stdout.lower()
    ...
    def test_list_filter_kind(self, runner, app, seeded_engine):
        ...
        assert "dmitry" not in result.stdout.lower()
...
class TestDelete:
    def test_delete_hard_no_attachments(self, runner, app, seeded_engine):
        from atlas.pm.db import make_session
        from atlas.pm.models import ActionLog, Participant
    ...
    def test_delete_force_cascades(self, runner, app, seeded_engine, projects_app):
        ...
        with make_session(seeded_engine) as session:
            ...
            task = Task(
                ...
                title="T", cpp_description="C", status="backlog", priority="P2",
            )
            session.add(task)
            session.commit()
            task_id = task.id
