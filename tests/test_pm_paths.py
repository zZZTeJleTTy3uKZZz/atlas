...
class TestPathBuilders:
    ...
    def test_archive_path_products(self):
        from atlas.pm.paths import archive_path
        root = Path("C:/PROJECT")
        assert archive_path(root, "products", "np-005") == Path(
            "C:/PROJECT/_Archive/products/np-005"
        )
    ...
    def test_group_path_business_product(self):
        from atlas.pm.paths import group_path
        root = Path("C:/PROJECT")
        assert group_path(root, "business-product", "np-005") == Path(
            "C:/PROJECT/Products/np-005"
        )
...
class TestExpectedProjectPath:
    ...
    def test_active_business_product(self):
        ...
        root = Path("C:/PROJECT")
        assert expected_project_path(
            root, "business-product", "np-005", archived=False
        ) == Path("C:/PROJECT/Products/np-005")
    ...
    def test_archived_products(self):
        ...
        root = Path("C:/PROJECT")
        assert expected_project_path(
            root, "business-product", "np-005",
            archived=True, archived_group="products",
        ) == Path("C:/PROJECT/_Archive/products/np-005")
...
class TestGetProjectsRoot:
    def test_get_projects_root_from_env(self, monkeypatch, tmp_path):
        from atlas.pm.paths import get_projects_root
