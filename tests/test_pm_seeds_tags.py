...
EXPECTED_OWNER_SLUGS = {"cifro-pro", "dmitry"}
...
def test_seed_base_tags_creates_all():
    ...
    from atlas.pm.seeds import seed_base_tags
...
def test_seed_base_tags_names_russian_ok():
    ...
    with _make_session() as session:
        ...
        dmitry = session.execute(
            select(Tag).where(Tag.slug == "dmitry")
        ).scalar_one()
        assert dmitry.name == "Дмитрий"
...
def test_seed_base_tags_partial_existing():
    ...
    with _make_session() as session:
        ...
        preexisting = [
            Tag(slug="dmitry", name="Pre-existing Dmitry", category="owner"),
        ...
        assert dmitry.name == "Pre-existing Dmitry"
