...
class TestSlugifyText:
    def test_russian_with_dot(self):
        from atlas.pm.slugs import slugify_text
        assert slugify_text("Cifro.pro портал") == "cifro-pro-portal"
...
class TestGeneratePrefixFromSlug:
    def test_single_word_takes_first_three(self):
        from atlas.pm.slugs import generate_prefix_from_slug
    ...
    def test_segmented_with_digits(self):
        # 'np-005' — первая буква каждого сегмента + цифры
        # фиксируем поведение: 'np-005' -> 'np5' (буквы букв-сегментов + единственная значимая цифра)
        from atlas.pm.slugs import generate_prefix_from_slug
        assert generate_prefix_from_slug("np-005") == "np5"
...
class TestGenerateUniqueSlug:
    def test_base_free_returns_base(self):
        from atlas.pm.slugs import generate_unique_slug
...
class TestBuildTaskSlug:
    def test_basic(self):
        from atlas.pm.slugs import build_task_slug
...
class TestNextTaskNumber:
    def test_empty_table_returns_1(self, fresh_db):
        from atlas.pm.slugs import next_task_number
...
class TestResolveProjectRef:
    ...
    def test_resolve_by_slug(self, fresh_db, seed_refs):
        from atlas.pm.slugs import resolve_project_ref
