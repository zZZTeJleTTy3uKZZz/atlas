...
def fake_run(monkeypatch):
    """Перехватываем `atlas.git_backend.run` — все subprocess через него."""
...
class TestGitInit:
    def test_init_creates_remote_and_updates_db(
        ...
        fake_run["set"](
            ...
            (0, "abc123\n", ""),                                      # git rev-parse HEAD
            (0, "", ""),                                              # git symbolic-ref
            (0, '{"id": 11, "full_path": "example-org/clients"}', ""),  # glab api groups/...
            (
                0,
        ...
        assert proj.git_remote_url == "https://gitlab.com/example-org/clients/cifro.git"
    ...
    def test_init_uses_personal_namespace_for_owner_owner(
        self, runner, app, seeded_engine, projects_root, fake_run
    ):
        ...
        _add_project(
            ...
            local_path=local,
            extra_args=["--tag", "owner:owner"],
        )

        fake_run["set"](
            (0, "", ""), (0, "", ""), (0, "", ""), (0, "abc\n", ""), (0, "", ""),
            (0, '{"id": 22, "full_path": "example-personal/products"}', ""),
            (
                0,
                '{"http_url_to_repo": "https://gitlab.com/example-personal/products/myutil.git"}',
                "",
            ),
            (0, "", ""), (0, "", ""),
        )
        ...
        # Должен быть вызов glab api groups/example-personal%2Fproducts
        flat = [" ".join(c["cmd"]) for c in fake_run["calls"]]
        assert any(
            "glab" in s and "example-personal" in s and "products" in s
            for s in flat
    ...
    def test_init_aborts_when_already_has_remote(
        ...
        _set_git_remote_url(
            ...
            "https://gitlab.com/example-org/clients/already",
        )
    ...
    def test_init_github_provider_uses_gh(
        self, runner, app, seeded_engine, projects_root, fake_run
    ):
        """#301: `git init --provider github --group <owner>` создаёт через gh."""
        local = _make_local_path(projects_root, "ghrepo")
        _add_project(
            runner, app,
            name="GH Repo", slug="ghrepo", type_slug="business-product",
            local_path=local,
        )
        # init, add, commit, rev-parse, symbolic-ref, gh repo create, remote add, push
        fake_run["set"](
            (0, "", ""), (0, "", ""), (0, "", ""), (0, "abc\n", ""), (0, "", ""),
            (0, "https://github.com/acme/ghrepo\n", ""),  # gh repo create
            (0, "", ""), (0, "", ""),
        )
        result = runner.invoke(
            app,
            ["git", "init", "ghrepo", "--provider", "github", "--group", "acme"],
        )
        assert result.exit_code == 0, _combined(result)

        proj = _get_project(seeded_engine, "ghrepo")
        assert proj.git_provider == "github"
        assert proj.git_remote_url == "https://github.com/acme/ghrepo.git"
        assert proj.git_repo_url == proj.git_remote_url

        cmds = [c["cmd"] for c in fake_run["calls"]]
        assert any(
            c[:3] == ["gh", "repo", "create"] and "acme/ghrepo" in c
            for c in cmds
        ), f"ожидался gh repo create, got: {cmds}"
        # glab НЕ должен вызываться для github-провайдера.
        assert not any(c and c[0] == "glab" for c in cmds)

    def test_init_rejects_unknown_provider(
        self, runner, app, seeded_engine, projects_root, fake_run
    ):
        local = _make_local_path(projects_root, "bb")
        _add_project(
            runner, app,
            name="BB", slug="bb", type_slug="business-product",
            local_path=local,
        )
        result = runner.invoke(
            app, ["git", "init", "bb", "--provider", "bitbucket"]
        )
        assert result.exit_code != 0
        assert "bitbucket" in _combined(result).lower()
...
class TestGitStatus:
    def test_status_prints_remote_and_local_info(
        ...
        _set_git_remote_url(
            seeded_engine, "stat",
            "https://gitlab.com/example-org/clients/stat",
        )
        ...
        assert result.exit_code == 0, _combined(result)
        out = _combined(result)
        assert "https://gitlab.com/example-org/clients/stat" in out
        assert "main" in out
    ...
    def test_status_no_git_dir_warns(
        ...
        _set_git_remote_url(
            seeded_engine, "nogit",
            "https://gitlab.com/example-org/clients/nogit",
        )
...
class TestGitPush:
    def test_push_calls_git_push_and_updates_timestamp(
        ...
        _set_git_remote_url(
            seeded_engine, "push1",
            "https://gitlab.com/example-org/clients/push1",
        )
    ...
    def test_push_aborts_when_no_local_path(
        ...
        _set_git_remote_url(
            ...
            "https://gitlab.com/example-org/clients/nopush",
        )
...
class TestGitLink:
    def test_link_attaches_existing_remote(
        ...
        url = "https://gitlab.com/example-org/clients/linkme"
...
class TestGitMove:
    def test_move_invokes_glab_transfer_and_updates_local_remote(
        ...
        _set_git_remote_url(
            seeded_engine, "mv1",
            "https://gitlab.com/example-org/clients/mv1",
        )
        ...
        fake_run["set"](
            (0, '{"id": 100, "path_with_namespace": "example-org/clients/mv1"}', ""),
            (0, '{"id": 200, "full_path": "example-org/archive/clients"}', ""),
            (
                0,
                '{"http_url_to_repo": "https://gitlab.com/example-org/archive/clients/mv1.git"}',
                "",
            ),
        ...
        result = runner.invoke(
            ...
            [
                "git", "move", "mv1",
                "--to-group", "example-org/archive/clients",
            ],
        )
        assert result.exit_code == 0, _combined(result)
...
class TestGitStatusAll:
    def test_status_all_lists_all_projects(
        ...
        _set_git_remote_url(
            seeded_engine, "p1",
            "https://gitlab.com/example-org/clients/p1",
        )
    ...
    def test_status_all_filters_by_type(
        ...
        _set_git_remote_url(
            seeded_engine, "c1",
            "https://gitlab.com/example-org/clients/c1",
        )
...
class TestGitSyncFromRemote:
    def test_sync_dry_run_does_not_modify_db(
        ...
        _set_git_remote_url(
            seeded_engine, "sync1",
            "https://gitlab.com/example-org/clients/sync1",
        )
        ...
        assert proj.git_remote_url == "https://gitlab.com/example-org/clients/sync1"
    ...
    def test_sync_updates_url_when_remote_changed(
        ...
        _set_git_remote_url(
            seeded_engine, "moved",
            "https://gitlab.com/example-org/clients/moved",
        )
        from atlas import git_backend
        ...
        def fake_status(self, repo_full_path):
            ...
            return {
                "web_url": "https://gitlab.com/example-org/archive/clients/moved",
                "default_branch": "main",
                "visibility": "private",
            }
