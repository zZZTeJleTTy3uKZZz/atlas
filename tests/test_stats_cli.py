...
def _prep(tmp_path):
    ...
    with make_session(engine) as s:
        ...
        s.add_all([t, st])
        s.flush()
        cp = s.query(Counterparty).filter(Counterparty.slug == "example-org").one()
...
def test_stats_overview_counterparty_breakdown(tmp_path):
    ...
    try:
        ...
        assert by_owner.get("example-org") == 1
    finally:
        _cleanup()
...
def test_dashboard_json_has_sections(tmp_path):
    """Новый операционный dashboard: --json несёт scope/projects/tasks/by_project."""
    _prep(tmp_path)
    try:
        ...
        assert "by_status" in data["tasks"] and "by_priority" in data["tasks"]
        assert "in_progress" in data["tasks"] and "blocked" in data["tasks"]
        assert "by_project" in data
    finally:
        _cleanup()


def test_dashboard_text_human_default(tmp_path):
    """Без --json — человеко-ориентированный Rich-вывод (дефолт у дашборда)."""
    _prep(tmp_path)
    try:
        res = runner.invoke(app, ["dashboard"])
        assert res.exit_code == 0, res.stdout
        assert "Atlas" in res.stdout  # Rich-шапка
    finally:
        _cleanup()


def test_dashboard_project_filter(tmp_path):
    _prep(tmp_path)
    try:
        res = runner.invoke(app, ["dashboard", "--json", "--project", "acme"])
        assert res.exit_code == 0, res.stdout
        data = json.loads(res.stdout)
