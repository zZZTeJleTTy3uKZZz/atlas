...
def test_checklist_event_wire_kind_is_checklist_item():
    ...
    ev = mapper.to_event(
        "create", "checklist", ci, portal_id="atlas-dmitry", parent_task=parent,
    )
...
def test_checklist_payload_core_fields():
    ...
    parent = SimpleNamespace(backend_id="task-be-1")
    ev = mapper.to_event(
        "update", "checklist", ci, portal_id="atlas-dmitry", parent_task=parent,
    )
    pl = ev["payload_json"]
...
def test_checklist_payload_done_false_from_zero():
    ci = _ci(is_done=0)
    ev = mapper.to_event(
        "create", "checklist", ci, portal_id="atlas-dmitry",
...
def test_checklist_parent_backend_id_none_when_no_parent():
    ...
    ci = _ci()
    ev = mapper.to_event("create", "checklist", ci, portal_id="atlas-dmitry")
