...
def test_install_profile_aware(monkeypatch, tmp_path):
    ...
    res = daemon.install(profile="owner", run=run)
    assert res["ok"] is True
    assert res["task"] == "atlas-sync-watch-owner"
    assert res["profile"] == "owner"
    assert "atlas-sync-watch-owner" in captured["script"]
    # профиль-специфичный VBS, в команде watch стоит --profile owner
    vbs = tmp_path / "scripts" / "sync_watch_headless-owner.vbs"
    assert vbs.exists()
    assert "--profile owner --text sync watch" in vbs.read_text(encoding="ascii")
