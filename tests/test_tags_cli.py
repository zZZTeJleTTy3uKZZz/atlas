"""Тесты для CLI `atlas tags ...` (PM-слой, Atlas).
...
def seeded_engine(fresh_engine):
    ...
    базовые теги (`b24`, `notion`, `owner` и пр.) создали бы коллизии slug/name.
    """
    from atlas.db import make_session
    from atlas.seeds import (
        seed_participants,
...
def app():
    """CLI-приложение tags."""
    from atlas.commands.tags import app as tags_app
    return tags_app
...
class TestList:
    ...
    def test_list_all(self, runner, app, seeded_engine):
        ...
        _add(runner, app, "--name", "Dmitry", "--category", "owner", "--slug", "owner-tag")
...
class TestGet:
    def test_get_by_category_slug(self, runner, app, seeded_engine):
        ...
        result = runner.invoke(app, ["get", "owner:owner-tag"])
        assert result.exit_code == 0, _combined(result)
        assert "owner-tag" in result.stdout.lower()

    def test_get_by_bare_slug(self, runner, app, seeded_engine):
        _add(runner, app, "--name", "Dmitry", "--category", "owner", "--slug", "owner-tag")

        result = runner.invoke(app, ["get", "owner-tag"])
        assert result.exit_code == 0, _combined(result)
        assert "owner-tag" in result.stdout.lower()
    ...
    def test_get_by_uuid_full(self, runner, app, seeded_engine):
        ...
        with make_session(seeded_engine) as session:
            t = session.execute(
                select(Tag).where(Tag.slug == "owner-tag")
            ).scalar_one()
            full_uuid = t.id
...
class TestDelete:
    def test_delete_unused(self, runner, app, seeded_engine):
        ...
        from atlas.db import make_session
        from atlas.models import ActionLog, Tag
    ...
    def test_delete_attached_with_force(self, runner, app, seeded_engine, projects_app):
        ...
        from atlas.db import make_session
        from atlas.models import ActionLog, Project, ProjectTag, Tag
