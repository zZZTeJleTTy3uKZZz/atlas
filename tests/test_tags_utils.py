...
def _attach(session, project_id: str, tag_id: str):
    from atlas.models import ProjectTag
...
class TestNormalizeTagRef:
    def test_qualified_owner(self):
        from atlas.tags import normalize_tag_ref
        assert normalize_tag_ref("owner:owner") == ("owner", "owner")
    ...
    def test_invalid_category_raises(self):
        ...
        from atlas.tags import InvalidTagCategoryError, normalize_tag_ref
        with pytest.raises(InvalidTagCategoryError):
            normalize_tag_ref("unknown:owner")
...
class TestGenerateTagSlug:
    def test_auto_from_name(self):
        from atlas.tags import generate_tag_slug
        assert generate_tag_slug("Owner", "owner", lambda _: False) == "owner"
    ...
    def test_collision_gets_suffix(self):
        from atlas.tags import generate_tag_slug
        taken = {"owner"}
        out = generate_tag_slug("Owner", "owner", lambda x: x in taken)
        assert out == "owner-2"
...
class TestResolveTagRef:
    def test_resolve_by_qualified(self, fresh_db):
        from atlas.tags import resolve_tag_ref

        t = _make_tag(fresh_db, slug="owner", name="Dmitry", category="owner")
        found = resolve_tag_ref(fresh_db, "owner:owner")
        assert found is not None
        assert found.id == t.id

    def test_resolve_by_qualified_wrong_category_returns_none(self, fresh_db):
        """owner:owner когда существует только stack:owner → None."""
        from atlas.tags import resolve_tag_ref

        _make_tag(fresh_db, slug="owner", name="Dmitry", category="stack")
        found = resolve_tag_ref(fresh_db, "owner:owner")
    ...
    def test_resolve_bare_slug_when_unique(self, fresh_db):
        ...
        t = _make_tag(fresh_db, slug="owner", name="Dmitry", category="owner")
        found = resolve_tag_ref(fresh_db, "owner")
        assert found is not None
        assert found.id == t.id
...
class TestListProjectTags:
    def test_empty_project_returns_empty(self, fresh_db, seed_refs):
        from atlas.tags import list_project_tags
...
class TestFilterProjectsByTags:
    def test_single_tag(self, fresh_db, seed_refs):
        from atlas.tags import filter_projects_by_tags
    ...
    def test_two_tags_and_semantics(self, fresh_db, seed_refs):
        ...
        t_owner = _make_tag(fresh_db, slug="owner", name="D", category="owner")

        # p1: оба тега; p2: только b24; p3: только owner
        _attach(fresh_db, p1.id, t_b24.id)
        _attach(fresh_db, p1.id, t_owner.id)
        _attach(fresh_db, p2.id, t_b24.id)
        _attach(fresh_db, p3.id, t_owner.id)

        result = filter_projects_by_tags(fresh_db, ["b24", "owner"])
        ids = {p.id for p in result}
        assert ids == {p1.id}
...
class TestAttachDetachTags:
    def test_attach_single_tag(self, fresh_db, seed_refs):
        from atlas.models import ProjectTag
        from atlas.tags import attach_tags
    ...
    def test_attach_multiple_tags(self, fresh_db, seed_refs):
        ...
        t2 = _make_tag(fresh_db, slug="owner", name="D", category="owner")
    ...
    def test_detach_single(self, fresh_db, seed_refs):
        from atlas.models import ProjectTag
        from atlas.tags import attach_tags, detach_tags
    ...
    def test_detach_idempotent(self, fresh_db, seed_refs):
        ...
        from atlas.tags import detach_tags
