...
@pytest.fixture()
def app():
    """CLI-приложение pm-tasks (+ lease/lifecycle-глаголы start/done/...)."""
    import atlas.commands.task_lease  # noqa: F401 — регистрирует claim/start/done/... на task_app
...
class TestAdd:
    def test_add_minimal(self, runner, app, project_cifro, seeded_engine):
        ...
        with make_session(seeded_engine) as session:
            ...
            assert task.status == "todo"  # новый дефолт (backlog убран)
    ...
    def test_add_with_assignee(self, runner, app, project_cifro, seeded_engine):
        from atlas.db import make_session
        from atlas.models import Participant, Task
...
class TestList:
    ...
    def test_list_filter_status(self, runner, app, project_cifro, seeded_engine):
        _add_task(runner, app, "--project", "cifro",
                  "--title", "Inb", "--cpp", "x", "--status", "todo")
        _add_task(runner, app, "--project", "cifro",
                  "--title", "Wip", "--cpp", "x")
        # in_progress ставится глаголом start (add --status его не принимает).
    ...
    def test_list_filter_assignee(self, runner, app, project_cifro, seeded_engine):
        _add_task(runner, app, "--project", "cifro",
                  "--title", "Mine", "--cpp", "x", "--assignee", "owner")
        _add_task(runner, app, "--project", "cifro",
                  "--title", "Free", "--cpp", "x")

        result = runner.invoke(app, ["list", "--assignee", "owner"])
        assert result.exit_code == 0
...
class TestGet:
    ...
    def test_get_shows_assignee(self, runner, app, project_cifro, seeded_engine):
        _add_task(runner, app, "--project", "cifro",
                  "--title", "X", "--cpp", "y", "--assignee", "owner")
        result = runner.invoke(app, ["get", "1"])
        assert result.exit_code == 0
        assert "owner" in result.stdout.lower() or "дмитрий" in result.stdout.lower()
...
class TestUpdate:
    ...
    def test_start_sets_in_progress_and_started_at(
        ...
        _add_task(runner, app, "--project", "cifro",
                  "--title", "X", "--cpp", "y")
        # in_progress теперь ставится глаголом start (= claim), не update --status.
    ...
    def test_reopen_via_update_clears_completed_at(
        self, runner, app, project_cifro, seeded_engine,
    ):
        from atlas.db import make_session
        from atlas.models import Task
        ...
        # реоткрытие в planning-статус (todo) разрешено update --status.
    ...
    def test_update_status_to_lifecycle_is_rejected(
        self, runner, app, project_cifro, seeded_engine,
    ):
        """Гейтинг: update --status in_progress/done/... запрещён (нужен глагол)."""
        _add_task(runner, app, "--project", "cifro",
                  "--title", "X", "--cpp", "y")
        for bad in ("in_progress", "done", "blocked", "review", "cancelled"):
            result = runner.invoke(app, ["update", "1", "--status", bad])
            assert result.exit_code != 0, f"{bad} должно быть отклонено"
...
class TestDelete:
    def test_delete_soft_sets_archived_at(self, runner, app, project_cifro, seeded_engine):
        from atlas.db import make_session
        from atlas.models import ActionLog, Task
...
def _make_epic(runner, epic_app, project: str, title: str) -> str:
    ...
    from atlas.db import resolve_db_url
    from atlas.models import Epic
...
class TestEpicLink:
    def test_add_epic_resolves_slug_to_epic_id(
        ...
        with make_session(seeded_engine) as session:
            ...
            # sprint_id вернулся как ОТДЕЛЬНЫЙ тайм-бокс (Scrum); --epic его НЕ трогает.
